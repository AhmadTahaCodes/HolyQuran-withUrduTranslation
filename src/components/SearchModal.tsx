  ...
  return (
    ...
    >
      ...
      >
        ...
        {onUpdatePageOffset && (
          <div className="px-4 py-2 bg-slate-100 dark:bg-slate-950/90 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
            <div className="flex items-center space-x-1.5">
              <Sliders className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
              <span>PDF Page Calibration:</span>
            </div>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[11px] font-bold transition-all ${
                    pageOffset === val
                      ...
                      : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-400 hover:bg-slate-300'
                  }`}
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3.5">
          ...
          {ayahMatch !== null && (
            <div
              ...
              className="flex items-center justify-between p-3.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-500/40 rounded-2xl hover:bg-emerald-100 dark:hover:bg-emerald-900/40 cursor-pointer transition-all shadow-sm group"
            >
              ...
              <div className="flex items-center space-x-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 group-hover:translate-x-1 transition-transform">
          ...
          {directPageNum !== null && (
            <div
              ...
              className="flex items-center justify-between p-3.5 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-500/30 rounded-2xl hover:bg-emerald-100 dark:hover:bg-emerald-900/30 cursor-pointer transition-all shadow-sm"
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-sm font-bold text-slate-800 dark:text-slate-100">Jump directly to Page {directPageNum}</div>
                  <div className="text-xs text-emerald-600 dark:text-emerald-400">Quran Page {directPageNum} of {TOTAL_PAGES}</div>
                </div>
              </div>
          ...
          <div>
            <div className="flex items-center justify-between text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 px-1">
              ...
              <span className="text-[11px] text-slate-400 font-mono">114 Surahs</span>
            </div>

            {matchingSurahs.length === 0 ? (
              <div className="text-xs text-slate-500 italic p-3">No Surahs match "{query}"</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {matchingSurahs.slice(0, 12).map((surah) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 rounded-xl hover:border-emerald-500/50 hover:bg-slate-100 dark:hover:bg-slate-800/70 cursor-pointer transition-all group"
                  >
                    <div className="flex items-center space-x-2.5 min-w-0">
                      <div className="w-7 h-7 rounded-lg bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-600 group-hover:text-white transition-colors flex-shrink-0">
                        {surah.id}
                      </div>
                      <div className="min-w-0">
                        <div className="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate">{surah.name_english}</div>
          ...
          {/* Juz Matches */}
          <div>
            <div className="flex items-center justify-between text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 px-1 pt-2">
              <span className="flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5 text-amber-500" />
                Juz Parts ({matchingJuz.length})
              </span>
              <span className="text-[11px] text-slate-400 font-mono">30 Juz</span>
            </div>

            {matchingJuz.length === 0 ? (
              <div className="text-xs text-slate-500 italic p-3">No Juz match "{query}"</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {matchingJuz.slice(0, 10).map((juz) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 rounded-xl hover:border-amber-500/50 hover:bg-slate-100 dark:hover:bg-slate-800/70 cursor-pointer transition-all"
                    >
                      <div className="flex items-center space-x-2.5 min-w-0">
                      ...
                      <div className="text-right flex-shrink-0 ml-2">
                        <div className="text-sm font-serif font-bold text-amber-600 dark:text-amber-400">الجُزْءُ {toArabicNumerals(juz.juz_number)} ({juz.name_arabic})</div>
                        <div className="text-[10px] text-slate-400 font-mono">Page {juz.start_page}</div>
                      </div>
                    </div>
                  );
                })}
