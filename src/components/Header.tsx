...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full h-14 bg-white/95 dark:bg-slate-900/95 sepia:bg-[#fbf5e6]/95 border-b border-slate-200 dark:border-slate-800/80 sepia:border-[#dfd3b9] backdrop-blur-md px-2.5 sm:px-4 flex items-center justify-between shadow-xs transition-colors select-none pt-[env(safe-area-inset-top,0px)]">
      {/* Left Section: Drawer Toggle & App Branding */}
      <div className="flex items-center space-x-1.5 sm:space-x-2">
        {/* Navigation Drawer Menu Button */}
        <button
          onClick={onOpenDrawer}
          className="h-9 px-2.5 text-slate-700 dark:text-slate-300 sepia:text-[#2d2417] hover:bg-slate-100 dark:hover:bg-slate-800 sepia:hover:bg-[#f2e9d2] rounded-xl transition-colors active:scale-[0.97] flex items-center space-x-1.5 border border-transparent hover:border-slate-200 dark:hover:border-slate-700/60"
        ...
        <button
          ...
          className="flex items-center space-x-2 p-1 text-left rounded-xl hover:opacity-90 transition-opacity active:scale-[0.98]"
          title="Go to Home"
        >
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-500 flex items-center justify-center text-white font-serif font-bold text-xs shadow-sm">
            قرآن
          </div>
      ...
      <div className="flex items-center">
        ...
        <div className="hidden md:flex items-center relative bg-slate-100 dark:bg-slate-950/80 sepia:bg-[#f2e9d2] p-1 rounded-2xl border border-slate-200 dark:border-slate-800/90 sepia:border-[#dfd3b9] text-xs shadow-xs select-none flex-shrink-0">
          ...
          <div
            className="absolute top-1 bottom-1 rounded-xl bg-emerald-600 shadow-sm transition-transform duration-200 ease-out pointer-events-none"
            style={{
        ...
        {/* Dynamic Surah & Page Controls (Smooth Animated Expand/Collapse for Reader) */}
        <div
          className={`overflow-hidden min-w-0 transition-all duration-200 ease-out flex items-center ${
            activeView === 'reader'
        ...
        >
          <div className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-950/80 sepia:bg-[#f2e9d2] border border-slate-200 dark:border-slate-800/80 sepia:border-[#dfd3b9] rounded-xl px-1.5 py-0.5 whitespace-nowrap min-w-max">
            <button
              onClick={() => onPageChange(activePage + (viewMode === 'dual' ? 2 : 1))}
              disabled={activePage >= TOTAL_PAGES}
              className="w-7 h-7 flex items-center justify-center text-slate-600 dark:text-slate-400 sepia:text-[#2d2417] hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-slate-200/60 dark:hover:bg-slate-800 rounded-lg disabled:opacity-25 transition-colors active:scale-[0.95]"
            ...
            <button
              onClick={() => onSelectView('reader')}
              className="text-center min-w-0 px-1.5 max-w-[120px] sm:max-w-[190px] hover:opacity-85 transition-opacity cursor-pointer"
      ...
      {/* Right Section: Standardized Action Buttons */}
      <div className="flex items-center space-x-1 sm:space-x-1.5">
        {/* PWA Install Button (When available) */}
        {canInstall && onInstallApp && (
          <button
            onClick={onInstallApp}
            className="flex items-center space-x-1 px-2.5 h-9 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold shadow-sm transition-colors active:scale-[0.97]"
